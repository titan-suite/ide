        ...
        <template slot-scope="scope">
          <el-popover v-model="scope.row.popoverOpen" trigger="hover" placement="left" width="200">
            ...
            <div style="text-align: right; margin: 0;padding:inherit">
              ...
              <el-button type="primary" size="mini" @click="handleUnlock(scope.row.address,scope.row.password);scope.row.popoverOpen = false">Confirm</el-button>
            </div>
            <el-button v-show="accounts.length>0" slot="reference" :type="scope.row.unlocked?'success':'info'" size="mini" @click="scope.row.password = ''">
...
export default class Accounts extends Vue {
    ...
    public async handleUnlock(address: string, password: string) {
        this.accountFetchLoading = true
